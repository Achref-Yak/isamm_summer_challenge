...
from  clubs.models import UserProfile, Club, Activity,Event, demander
...
def event_view(request,string):
	...
	if  request.method == 'POST':
		...
		args = {'event':event,'date':date.today,'nom_de_club':club,'nom_de_event':string,'activity':activity,'club_form':club,'calenderier':cal}
		return render(request, template_name,args)	
...
def club_view(request,string):
	...
	if request.method == 'POST' and 'invi' in request.POST:
 
		b = demander(user_from=request.user,user_from_name=user_from_name,user_to=club.superviseur,accepted=False,invi_type="club",club=club)
		b.save()
		...
		event = EventForm()
		demande = InviForm()
		args = {'club_form':club_form,'user':user,'event':event,'demande':demande, 'events':events,'activity':activity,'nb_event':nb_event}
		return render(request, template_name,args)	
